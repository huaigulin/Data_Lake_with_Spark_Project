...
def process_song_data(spark):
    """Reads song data files from S3, extracts and transforms data for songs and artists tables, and load them to a new S3 bucket
    ...
    # extract columns to create songs table, make necessary type casts and remove duplicates
    ...
    # extract columns to create artists table, make necessary type casts, remove duplicates and rename columns
...
def process_log_data(spark):
    """Reads log data files from S3, extracts and transforms data for users, time and songplays tables, and load them to a new S3 bucket
    ...
    # create timestamp column from original ts column with a udf
    get_timestamp = udf(lambda x: datetime.fromtimestamp(x / 1000.0), TimestampType())
    df = df.withColumn("timestamp", get_timestamp(df['ts']))

    # create datetime column from original ts column
    ...
    songplays_table = song_df.join(df, (song_df['artist_name'] == df['artist']) & (song_df['title'] == df['song'])) \
        [[col('timestamp').alias('start_time'), col('userId').alias('user_id').cast(IntegerType()), 'level', 'song_id', \
            'artist_id', col('sessionId').alias('session_id').cast(IntegerType()), 'location', col('userAgent').alias('user_agent')]] \
